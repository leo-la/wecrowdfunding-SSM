...
import com.leo.manager.service.impl.AuthServiceImpl;
...
public class AuthController {
    ...
    public String toAccountType(Model model){
        Integer authCount = authService.findAuthCount();
    ...
    public String toAuthDer(Model model){
        Member member = userService.findMemberById(ApplicationUtils.getUserInfo().getId());
    ...
    public String toApplyDer(Model model){
        List<Cert> certs = businessService.findCerts();
        List<Integer> certids = businessService.findTypeCert();
    ...
    public String toEmailDer(Model model){
        String email = ApplicationUtils.getUserInfo().getEmail();
    ...
    public String toAuth(Model model){
        Integer status = authService.findStatus();
        Member member = userService.findMemberById(ApplicationUtils.getUserInfo().getId());
        model.addAttribute("member",member);
        if(status==0){
            return "accttype";
    ...
    public String applyNext(Member member){
        authService.updateStatus(ApplicationUtils.getUserInfo().getId(),"3");
        member.setId(ApplicationUtils.getUserInfo().getId());
    ...
    public String authAccttype(@PathVariable String title){
        title = title.substring(1,title.length()-1);
        authService.addAuthAccttype(title);
        return "redirect:/auth/toAuthDer";
    }
    ...
    public boolean updateAvatar(MultipartFile file,Integer certid){
        boolean isSuccess = false;
        try{
            String filename = file.getOriginalFilename();
            filename = UUID.randomUUID().toString().replace("-","") + "_" + filename;
            String realPath = staticCertsPath;
            isSuccess = authService.updateAuthCertImg(certid,filename);
    ...
    public boolean starProcess(AuthProcess authProcess){
        ...
        String code1 = authService.findCode();
        if(code1==null){
            ...
            return authService.updateProcess(authProcess);
        }else {
            LogUtils.getLogger().info("发送验证码");
            MailUtils.sendMail(ApplicationUtils.getUserInfo().getEmail(),code,"验证码");
            return authService.updateCode(authProcess);
        }
    ...
    public boolean apply(@RequestBody String incode){
        incode = incode.split(":")[1].substring(1,incode.split(":")[1].length()-2);
        AuthProcess auth = authService.findAuthByMid();
