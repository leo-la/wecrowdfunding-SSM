...
public class UserController {
    ...
    public String toUser(HttpServletRequest request, Model model, Principal principal){
        model.addAttribute("path",staticAccessPath);

        String name = principal.getName();
        Member member = userService.findMemberByName(name);
    ...
    public String login(HttpServletRequest request){
        ...
        Member member = userService.findMemberByName(userDetails.getUsername());
        request.getSession().setAttribute("user",member);
    ...
    /**
     * 显示图片
     * 单服务器采用该方式加载图片，nginx-tomcat集群则采用动静分离
     * @param image_name
     * @return
     * @throws Exception
     */
